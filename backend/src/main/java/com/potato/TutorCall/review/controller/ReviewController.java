...
public class ReviewController {
  ...
  public ResponseEntity<?> saveTutorCallReview(
      @PathVariable("tutorCallId") Long id,
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(new CreatedResponseDto(reviewId, "리뷰가 등록되었습니다."));
  }

  @PostMapping("/lecture/{lectureId}")
  public ResponseEntity<?> saveLectureReview(
      @PathVariable("lectureId") Long id, @RequestBody ReviewRequestDto dto, HttpSession session) {
    UserSessionDto userSessionDto = (UserSessionDto) session.getAttribute(SessionKey.USER);
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(new CreatedResponseDto(id, "리뷰가 등록되었습니다."));
  }

  @PatchMapping("/{reviewId}")
  public ResponseEntity<?> updateReview(
      @PathVariable("reviewId") Long id, @RequestBody ReviewRequestDto dto, HttpSession session) {
    UserSessionDto userSessionDto = (UserSessionDto) session.getAttribute(SessionKey.USER);
