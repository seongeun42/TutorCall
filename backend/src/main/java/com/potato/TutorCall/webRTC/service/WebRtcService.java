...
public class WebRtcService {
   ...
   */
  public String createSession(Long id, StudyType type, OpenVidu openVidu)
   ...
   * @return 연결 토큰
   */
  public String getConnection(Long id, Long userId, StudyType type, OpenVidu openVidu)
      throws OpenViduJavaClientException, OpenViduHttpException {
    if (redisTemplate.opsForValue().get(type.getValue() + id) == null) {
    ...
    redisTemplate
        .opsForHash()
        .putIfAbsent(
            session.getSessionId() + "participant", "user" + userId, connection.getConnectionId());
   ...
   * @return 세션 삭제 여부
   */
  public boolean removeConnection(Long id, Long userId, StudyType type, OpenVidu openVidu)
      throws OpenViduJavaClientException, OpenViduHttpException {
    if (redisTemplate.opsForValue().get(type.getValue() + id) == null) {
      throw new NotFoundException("존재하지 않는 세션입니다.");
    }
