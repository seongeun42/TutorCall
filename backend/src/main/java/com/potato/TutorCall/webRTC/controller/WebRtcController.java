...
public class WebRtcController {
  ...
  public ResponseEntity<?> createLectureRoom(
      @PathVariable("lectureId") Long lectureId, HttpSession session)
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(new CreateSessionResponseDto(roomId, "과외룸이 생성되었습니다."));
  }
  ...
  public ResponseEntity<?> createTutorCallRoom(@PathVariable("tutorcallId") Long tutorCallId)
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(new CreateSessionResponseDto(roomId, "튜터콜룸이 생성되었습니다."));
  }

  @PostMapping("/lecture/{lectureId}/connection")
  public ResponseEntity<?> createLectureConnection(
      @PathVariable("lectureId") Long lectureId, HttpSession session)
      throws OpenViduJavaClientException, OpenViduHttpException {
    UserSessionDto userSessionDto = (UserSessionDto) session.getAttribute(SessionKey.USER);
    Lecture lecture = lectureService.findById(lectureId);
    ...
    String token =
        webRtcService.getConnection(lectureId, userSessionDto.getId(), StudyType.LECTURE, openvidu);
    log.info(userSessionDto.getId() + "님의 " + lectureId + "과외 화상강의룸 세션 연결에 성공했습니다.");
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(new ConnectionResponseDto(token, "연결에 성공했습니다."));
  }

  @PostMapping("/tutorcall/{tutorcallId}/connection")
  public ResponseEntity<?> createTutorCallConnection(
      @PathVariable("tutorcallId") Long tutorCallId, HttpSession session)
      throws OpenViduJavaClientException, OpenViduHttpException {
    UserSessionDto userSessionDto = (UserSessionDto) session.getAttribute(SessionKey.USER);
    TutorCall tutorCall = tutorCallService.findById(tutorCallId);
    if (!userSessionDto.getId().equals(tutorCall.getTutor().getId())
        && !userSessionDto.getId().equals(tutorCall.getUser().getId())) {
      throw new ForbiddenException("입장 권한이 없습니다.");
    }
    String token =
        webRtcService.getConnection(
            tutorCallId, userSessionDto.getId(), StudyType.TUTORCALL, openvidu);
  ...
  public ResponseEntity<?> removeLectureConnection(
      @PathVariable("lectureId") Long lectureId, HttpSession session)
      throws OpenViduJavaClientException, OpenViduHttpException {
    UserSessionDto userSessionDto = (UserSessionDto) session.getAttribute(SessionKey.USER);
    if (webRtcService.removeConnection(
        lectureId, userSessionDto.getId(), StudyType.LECTURE, openvidu)) {
  ...
  public ResponseEntity<?> removeTutorCallConnection(
      @PathVariable("tutorcallId") Long tutorCallId, HttpSession session)
      throws OpenViduJavaClientException, OpenViduHttpException {
    UserSessionDto userSessionDto = (UserSessionDto) session.getAttribute(SessionKey.USER);
    if (webRtcService.removeConnection(
