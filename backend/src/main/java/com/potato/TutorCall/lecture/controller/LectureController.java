...
public class LectureController {
  ...
  public ResponseEntity<?> getLectureList(
    ...
    LectureSearchCondition condition =
        LectureSearchCondition.builder().keyword(keyword).tag(tag).state(state).build();
  ...
  public ResponseEntity<?> getLecturePromotion(
      @PathVariable("lectureId") Long lectureId, HttpSession session) {
  ...
  public ResponseEntity<?> registLecture(
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(new CreatedResponseDto(id, "과외에 등록되었습니다."));
  }

  @DeleteMapping("/participant/{lectureId}")
  public ResponseEntity<?> unRegistLecture(
      @PathVariable("lectureId") Long lectureId, HttpSession session) {
    UserSessionDto userSessionDto = (UserSessionDto) session.getAttribute(SessionKey.USER);
    Lecture lecture = lectureService.findById(lectureId);
  ...
  public ResponseEntity<?> changePromotionState(
      @PathVariable("lectureId") Long lectureId,
      @RequestBody PromotionStateRequestDto dto,
      HttpSession session) {
    UserSessionDto userSessionDto = (UserSessionDto) session.getAttribute(SessionKey.USER);
    if (dto.getState() == null) throw new InvalidException("상태 정보가 존재하지 않습니다.");
  ...
  public ResponseEntity<?> changeLectureTerm(
      @PathVariable("lectureId") Long lectureId,
  ...
  private ResponseEntity<?> getLecture(
      @PathVariable("lectureId") Long lectureId, HttpSession session) {
    UserSessionDto userSessionDto = (UserSessionDto) session.getAttribute(SessionKey.USER);
    Lecture lecture = lectureService.findById(lectureId);
