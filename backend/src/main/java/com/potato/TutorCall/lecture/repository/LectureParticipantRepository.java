...
public interface LectureParticipantRepository extends JpaRepository<LectureParticipant, Long> {
  ...
  @Query(
      value =
          "select count(lp) > 0 from LectureParticipant lp where lp.lecture.id = :lectureId and"
              + " lp.user.id = :userId")
  Boolean existByLectureAndUser(
      @Param(value = "lectureId") Long lectureId, @Param(value = "userId") Long userId);
  ...
  @Query(
      value =
          "delete from LectureParticipant lp where lp.lecture.id = :lectureId and  lp.user.id ="
              + " :userId")
  void deleteByLectureAndUser(
      @Param(value = "lectureId") Long lectureId, @Param(value = "userId") Long userId);
