...
public class LectureParticipantService {
  ...
  public Long registLecture(Lecture lecture, User user) {
    ...
    return lpRepository
        .save(LectureParticipant.builder().lecture(lecture).user(user).build())
        .getId();
  }
  ...
  public void unRegistLecture(Lecture lecture, Long userId) {
    ...
    if (lecture.getParticipants() <= 0) throw new OutOfRangeException("이미 최소 인원입니다.");
    lecture.changeParticipants(-1);
  ...
  public LectureParticipant findById(Long id) {
    return lpRepository.findById(id).orElseThrow(() -> new NotFoundException("존재하지 않는 등록 내역입니다."));
  }
