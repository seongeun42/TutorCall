...
public class LectureService {
  ...
  public Lecture findById(Long id) {
    Lecture lecture =
        lectureRepository.findById(id).orElseThrow(() -> new NotFoundException("존재하지 않는 과외입니다."));
    if (lecture.isDelete()) throw new ForbiddenException("삭제된 과외입니다.");
    return lecture;
  }
  ...
  @Transactional(readOnly = true)
  public Page<LectureListResponseDto> getLectureList(
      LectureSearchCondition condition, Pageable pageable) {
    return lectureSearchRepository.search(condition, pageable);
  }

  public List<Lecture> findUserLectures(User user) {
    return lectureParticipantRepository.findLectureByUserOrderByIdDesc(user).stream()
        .map(LectureParticipant::getLecture)
        .toList();
  }

  public void changePromotionState(Long lectureId, Tutor tutor, Boolean state) {
    Lecture lecture = this.findById(lectureId);
    if (!lecture.getTutor().equals(tutor)) throw new ForbiddenException("상태 변경 권한이 없습니다.");
    lecture.changePromotionState(state);
  }

  public void changeLectureTerm(
      Long lectureId, Tutor tutor, LocalDateTime start, LocalDateTime end) {
    Lecture lecture = this.findById(lectureId);
    if (!lecture.getTutor().equals(tutor)) throw new ForbiddenException("기간 변경 권한이 없습니다.");
  ...
  public void changePromotionState() {
    ...
    lectureRepository.changeLecturePromotionState(
        yesterday.atTime(0, 0, 0), yesterday.atTime(23, 59, 59));
  }

  public void changeLectureState() {
    LocalDate yesterday = LocalDate.now().minusDays(1);
    lectureRepository.changeLectureState(yesterday.atTime(0, 0, 0), yesterday.atTime(23, 59, 59));
  }
