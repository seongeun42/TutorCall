...
import com.potato.TutorCall.qna.exception.CustomResponseException;
...
public class QuestionService {
    ...
    public ResponseEntity<?> question(int questionId){
        ...
        Question q= questionRepository.findById((long) questionId)
                .orElseThrow(()-> new CustomResponseException(HttpStatus.BAD_REQUEST, "질문 조회 실패"));
    ...
    public ResponseEntity<?> writeQuestion(QuestionWriteDto questionWriteDto){
        ...
        Tag tag = tagRepository.findById(questionWriteDto.getTagId())
                .orElseThrow(()-> new CustomResponseException(HttpStatus.BAD_REQUEST, "질문 작성 실패"));
        ...
        if (questionId == null) throw new CustomResponseException(HttpStatus.BAD_REQUEST, "질문 작성 실패");
    ...
    public ResponseEntity<?> editQuestion(int questionId, QuestionWriteDto questionWriteDto){
        ...
        CommonResponseDto commonResponseDto;
        Tag tag = tagRepository.findById(questionWriteDto.getTagId())
                .orElseThrow(()-> new CustomResponseException(HttpStatus.BAD_REQUEST, "질문 수정 실패"));
        User user = userRepository.findById(questionWriteDto.getWriterId())
                .orElseThrow(()-> new CustomResponseException(HttpStatus.BAD_REQUEST, "질문 수정 실패"));
        ...
        if(!editTarget.getId().equals(user.getId()))
            throw new CustomResponseException(HttpStatus.NOT_ACCEPTABLE, "수정 권한 없음");

        count = questionRepository.editQuestion(questionId, questionWriteDto, user, tag);
        if(count == 0)  throw new CustomResponseException(HttpStatus.BAD_REQUEST, "질문 수정 실패");
    ...
    public ResponseEntity<?> deleteQuestion(int questionId, long writerId){
        ...
        if (count ==0) throw new CustomResponseException(HttpStatus.BAD_REQUEST, "질문 삭제 실패");
