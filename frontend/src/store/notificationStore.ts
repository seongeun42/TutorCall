...
export const useNotificationStore = defineStore('notification', {
    state: () => ({
        ...
        // 매칭된 강의룸 세션
        roomSessionId: '' as String | null,
        // 현재 요청한 tutorcall uuid
        requestUuid: '' as UUID | null,
        // 현재 수락한 response uuid
        responseUuid: '' as UUID | null
    }),
    actions: {
        ...
        callSubscribe(reqUuid: UUID) { // 튜터콜 요청 후 수락 구독
            if (this.stompClient) {
                this.requestUuid = reqUuid
                let callSub: Stomp.Subscription | null =  this.stompClient.subscribe("/sub/tutorcall/" + reqUuid, (response: any) => {
                    // 수락을 수락함에 저장
        ...
        answerSubscribe(resUuid: UUID, reqUuid: UUID | null) { // 수락 후 매칭 구독
            if (this.stompClient) {
                this.waitingMatching = true
                this.requestUuid = reqUuid
                let answerSub: Stomp.Subscription | null = this.stompClient.subscribe("/sub/tutorcall/answer/" + resUuid, (response: any) => {
                    const data: matchedResponse = JSON.parse(response.body)
                    if (data.matched) { // 매칭 성공
...
function checkoverlapping(objectsize:number, positionX:number, positionY:number, arr:acceptTutor[]):boolean
{
  const length = arr.length;

  for(let i =0; i<length-1; i++){
    const r = Math.sqrt(Math.pow(Math.abs(arr[i].positionX-positionX),2)+Math.pow(Math.abs(arr[i].positionY-positionY),2))
